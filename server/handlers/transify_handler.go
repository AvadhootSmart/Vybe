...
func Transify(c *fiber.Ctx) error {
	...
	for _, videoId := range req.VideoIDs {
		...
		go func(videoId string) {
			...
			finalPath := checkOrDownloadVideo(ctx, videoId, cacheDir)
			if finalPath != "" {
				mu.Lock()
				cachedFiles[videoId] = finalPath
				mu.Unlock()
...
// checkOrDownloadVideo checks if file exists locally, otherwise downloads it
func checkOrDownloadVideo(ctx context.Context, videoId, cacheDir string) string {
	finalPath := filepath.Join(cacheDir, fmt.Sprintf("%s.m4a", videoId))

	// Step 1: Check if file already exists
	if _, err := os.Stat(finalPath); err == nil {
		log.Printf("File already exists for %s at %s", videoId, finalPath)
		return finalPath
	}
